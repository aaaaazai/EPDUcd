...
class RandomHorizontalFlip1(object):
    ...
    def __call__(self, imageA, imageB, label, grad_label):
        if random.random() < self.flip_prob:
            imageA = F.hflip(imageA)
            imageB = F.hflip(imageB)
            label = F.hflip(label)
            grad_label = F.hflip(grad_label)
        return imageA, imageB, label, grad_label
...
class ToTensor1(object):
    def __call__(self, imageA, imageB, label, grad_label):
        ...
        grad_label = F.to_tensor(grad_label)
        return imgA, imgB, label, grad_label
...
class Compose1(object):
    ...
    def __call__(self, imageA, imageB, label, grad_label):
        for t in self.transforms:
            imageA, imageB, label,grad_label = t(imageA, imageB, label,grad_label)
...
def load_img(root_dir):
    ...
    class MyDataset(Dataset):
        def __init__(self, phase, transform=None):  # __init__ 方法初始化数据集的路径，并列出所有图像路径。
            ...
            self.grad_label_dir = os.path.join(self.phase_dir, 'grad_label')
            ...
            self.label_paths = [os.path.join(self.label_dir, f) for f in os.listdir(self.label_dir) if
                                f.endswith('.jpg')]
            self.grad_label_paths = [os.path.join(self.grad_label_dir, f) for f in os.listdir(self.grad_label_dir) if
                                     f.endswith('.jpg')]
        ...
        def __getitem__(self, idx):  # __getitem__ 方法根据索引加载并返回一个批次的图像和标签。
            imgA_path, imgB_path, label_path, grad_label_path = self.imgA_paths[idx], self.imgB_paths[idx], \
            self.label_paths[idx], self.grad_label_paths[idx]
            ...
            grad_label = Image.open(grad_label_path).convert('L')  # 标签通常使用灰度图

            if self.transform:
                imgA, imgB, label, grad_label = self.transform(imgA, imgB, label, grad_label)

            return imgA, imgB, label, grad_label

    # 创建数据集和数据加载器
    image_datasets = {x: MyDataset(x, data_transforms[x]) for x in ['train', 'test', 'val']}
    batch_size = 2  # 根据显存大小和需求设置
...
if __name__ == "__main__":
    root_dir = './datasets/LEVIRCD_FC'  # 确保路径正确
    train_loader, test_loader, val_loader, batch_size = load_img(root_dir)

    # 测试训练数据加载
    for i, (imagesA, imagesB, labels,grad_label) in enumerate(train_loader):
        print(
            f"Batch {i + 1}: imagesA shape: {imagesA.size()}, imagesB shape: {imagesB.size()}, labels shape: {labels.size()},grad_label shape: {grad_label.size()}")
        if i == 0:
            break  # 只打印第一个批次的数据
